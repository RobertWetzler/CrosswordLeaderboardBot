        ...
        if len(rank) == 0:
            ...
            for name in globalChatData[chatID]['leaderboard']:
                if len(totalRank) == 0:
                    ...
                    inserted = False
                    while i < len(totalRank) and not(inserted):
                        if globalChatData[chatID]['leaderboard'][name] > globalChatData[chatID]['leaderboard'][totalRank[i][0]]:
                            totalRank.insert(i, [name])
                            inserted = True
                        elif globalChatData[chatID]['leaderboard'][name] == globalChatData[chatID]['leaderboard'][totalRank[i][0]]:
                            totalRank[i].append(name)
                            inserted = True
                        else:
                            i += 1
...
def currentstandings(update, context):
    ...
    if len(rank) == 0:
        ...
        if len(context.chat_data['daily']) == 1 or not('pinnedStandings' in context.chat_data):
            context.chat_data['pinnedStandings'] = context.bot.send_message(update.message.chat_id, mg, parse_mode=ParseMode.HTML)
            context.bot.pinChatMessage(update.message.chat_id, context.chat_data['pinnedStandings'].message_id, True)
        else:
            context.bot.edit_message_text(mg, chat_id = update.message.chat_id, message_id=context.chat_data['pinnedStandings'].message_id, parse_mode=ParseMode.HTML)
...
def insultmax(update, context):
    insults = ["Max sux", "Max is a doodee poopoo head", "Ew", "Did someone just fart? Oh nevermind, it was Max", "Max sucks at crossword", "Max is a poop poop poopy poop poop doodoodoodoodoodoodoo fart."]
...
def main():
    ...
    updater = Updater("797998516:AAGsNEGyns0PLaAgYZFl-PvSpFSJbtsPMLw", persistence=botPersistence, use_context=True)
